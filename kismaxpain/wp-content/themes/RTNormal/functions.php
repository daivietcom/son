			...
			echo '<div class="wrap">';
				if ( count( $logo ) > 0 && ! empty( $logo ) ) {
					echo '<div class="header-logo">';
						echo "<a class='banner' href='{$linkhome}' title='".get_bloginfo('name')."'><img src='". $logo['url'] ."' alt='".get_bloginfo('name')."' /></a>";
					echo '</div>';	
				}
...
function add_raothue_link(){
	echo "<p id='credit-link'>
			<a rel='nofollow' target='_blank' href='http://dvchat.vn/' title='thiet ke website' >
			<strong>Design by DV chat </strong>
			</a>
		</p>";
 ...
 * Add Thumb Size
**/
add_image_size( 'rt_thumb405x422', 405, 422, array( 'center', 'center' ) );
add_image_size( 'rt_thumb300x184', 300, 184, array( 'center', 'center' ) );
