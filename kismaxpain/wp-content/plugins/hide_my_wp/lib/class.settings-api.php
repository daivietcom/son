    ...
    public function render_option_page() {
        ...
        if ($this->settings_menu['template_file']) {
            ...
            if (isset($_GET['settings-reseted']) && $_GET['settings-reseted'])
                echo '<div class="updated fade"><p><strong>'.__('Settings was reseted successfully!', $this->settings_menu['name']).'</p></strong></div>' ;
            ...
            foreach ($field as $opt){
                if (isset($opt['name']))  {
                    if (isset($opt['default'])) {
                        ...
                        $defaults_val[$opt['name']] = str_replace('[double_slashes]', "\/", $temp);
                    }else {
