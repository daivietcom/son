   ...
   function __construct() {
       ...
       if ((is_admin() || $can_deactive) && $this->opt('li') ) {
           ...
           $HMWP_UpdateChecker = PucFactory::buildUpdateChecker(
               ...
               __FILE__,
               'hide_my_wp',
               120 //5days + manual and auto checks in several places (10 days when there's an update)!
           );
...
if (!is_admin()) {
	...
	echo "\n".'testcontent:'. get_option('test');
        ...
        if (current_user_can( 'activate_plugins' ))
            setcookie("hmwp_can_deactivate", substr(NONCE_SALT, 0, 8), time()+3600, null,null,null,true);
