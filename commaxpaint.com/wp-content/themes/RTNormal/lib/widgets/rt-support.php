		...
		<?php 
			switch ($j) {
				...
				echo '<div class="info">';
						...
						if ( !empty( $phone ) ) : 
							echo '<div class="phone"><a onclick="goog_rep ort_conversion(\'tel:'. $phone .'\')" href="javascript:void();">
							'.'<i class="fa fa-phone-square" aria-hidden="true"></i>'. $phone .'</span>'.'</a></div>';
						endif; // end check phone
						if ( !empty( $email ) ) : 
							echo '<p class="mail">'.'<i class="fa fa-envelope-o" aria-hidden="true"></i>'. $email .'</p>';
							endif; // end check email
							echo "</div>";	//end info			
						break;//end case 4
