...
function add_raothue_link(){
	echo "<p id='credit-link'>
			<a rel='nofollow' target='_blank' href='https://dvchat.vn/' title='' >
			<strong>Design by DVChat </strong>
			</a>
		</p>";
 ...
 * Add Thumb Size
**/
add_image_size( 'rt_thumb270x150', 270, 150, array( 'center', 'center' ) );
add_image_size( 'rt_thumb400x256', 400, 256, array( 'center', 'center' ) );
...
function add_date_time(){
	$html .='<p class="time-count">';
	$html .= '<span class="date-time"><i class="fa fa-calendar-check-o" aria-hidden="true"></i>' . get_the_time('d/m/Y') . '</span>';
	$html .= '<span class="red"><i class="fa fa-commenting" aria-hidden="true"></i>'. get_post_views (get_the_ID()).'</span>';
	$html .='</p>';
	return $html;
}