...
class Gtid_Support_Online extends WP_Widget{
	function Gtid_Support_Online(){
		$widget_ops                = array(
			...
			'description'          => __('Add nick yahoo , skype on website',RT_LANGUAGE)
		);
		$control_ops               = array(
			'width'                => 505,
			'height'               => 250,
		...
		$this ->WP_Widget('support_online',__('support online',RT_LANGUAGE),$widget_ops,$control_ops);
	}
	/***Gtid_Support_Online*/
					...
					<?php
						echo '<div class="info">';
							echo '<div class="support-rt">';
								if ( !empty( $name ) )  {
									echo '<span class="name-support"><span>'.$name.'</span></span>';
								}
								if ( !empty( $phone ) ) {
									echo '<span class="phone-support phone-support_2 phone_support_3"><a href= tel:'.$phone.' >'.$phone.'</a></span>';
								}
									if ( in_array( $giaodien, array( 3,4,5,6 ) ) ) {
	...
	function form( $instance ){
		...
		?>
		<p>
			<label for="<?php echo $this->get_field_id('number_supporter');?>"><?php _e( 'Number support','genesis' );?></label>
			<input type="number" id="<?php echo $this->get_field_id('number_supporter');?>" name="<?php echo $this->get_field_name('number_supporter') ;?>" value="<?php echo esc_attr( $instance['number_supporter'] );?>" style="width:15%" path="note" />
			<label for="<?php echo $this->get_field_id('giaodien');?>"><?php _e( 'Style','genesis' );?>:</label>
			<select id="<?php echo $this->get_field_id('data_style');?>" name="<?php echo $this->get_field_name('data_style') ;?>" style="width:150px;">
				<?php
					for ($i=1; $i < 7 ; $i++): ?>
			...
			<input type="submit" name="save-widget" class="button-primary widget-control-save" value="save" />
		</p>
		<?php
	...
	function rt_support_title( $title, $value ){ ?>
			<p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e( 'Title', RT_LANGUAGE ); ?>:</label>
				<input type="text" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" value="<?php echo esc_attr( $value ); ?>" style="width:100%;"
				/></p>
		<?php 
	}
	function rt_support_img( $link_img, $value ){ 
		...
		?>
			...
			<p class="link-img"><label for="<?php echo $this->get_field_id('link_img'); ?>"><?php _e( 'Link_img', RT_LANGUAGE ); ?>:</label>
				<input type="text" id="<?php echo $this->get_field_id('link_img'); ?>" name="<?php echo $this->get_field_name('link_img'); ?>" value="<?php echo esc_attr( $value ); ?>" style="width:100%;" <?php if ( $username != 'adminraothue' ) echo 'disabled'; ?>
				/></p>
		<?php
	}
	function rt_support_name( $name, $value, $i ){ ?>
			<p><label for="<?php echo $this->get_field_id( $name ); ?>"><?php _e( 'Name-'. $i .'', RT_LANGUAGE ); ?>:</label>
	...
	function rt_support_mail( $mail, $value, $i ){ ?>
			<p><label for="<?php echo $this->get_field_id( $mail ); ?>"><?php _e( 'Mail-'. $i .'', RT_LANGUAGE ); ?>:</label>
	...
	function rt_support_mail2( $mail2, $value, $i ){ ?>
		<p><label for="<?php echo $this->get_field_id( $mail2 ); ?>"><?php _e( 'Mail-'. $i .'', RT_LANGUAGE ); ?>:</label>
	...
	function rt_support_phone( $phone, $value, $i ){ ?>
		<p><label for="<?php echo $this->get_field_id( $phone ); ?>"><?php _e( 'Phone number-'.$i.'', RT_LANGUAGE ); ?>:</label>
		<input type="number" id="<?php echo $this->get_field_id( $phone ); ?>" name="<?php echo $this->get_field_name( $phone ); ?>" value="<?php echo esc_attr( $value ); ?>" style="width:100%;" /></p>
		<?php
	 }
	function rt_support_skype( $skype, $value, $i ){ ?>
		<p><label for="<?php echo $this->get_field_id( $skype ); ?>"><?php _e( 'Skype ID ', RT_LANGUAGE ); ?>:</label>
	...
	function rt_support_phone_2( $phone2, $value, $i ){ ?>
		<p><label for="<?php echo $this->get_field_id( $phone2 ); ?>"><?php _e( 'Phone number 2', RT_LANGUAGE ); ?>:</label>
	...
	function rt_support_zalo( $zalo, $value, $i ){ ?>
		<p><label for="<?php echo $this->get_field_id( $zalo ); ?>"><?php _e( 'Zalo ', RT_LANGUAGE ); ?>:</label>
	...
	function rt_support_viber( $viber, $value, $i ){ ?>
		<p><label for="<?php echo $this->get_field_id( $viber ); ?>"><?php _e( 'viber ', RT_LANGUAGE ); ?>:</label>
