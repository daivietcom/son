...
if ( ! function_exists( 'rt_woocommerce_get_product_thumbnail' ) ) {
	...
	function rt_woocommerce_get_product_thumbnail( $size = 'shop_catalog', $deprecated1 = 0, $deprecated2 = 0 ) {
		global $post, $rt_option;
		$wg = $rt_option['on_cart_image_width'];
		$hg = $rt_option['on_cart_image_heigh'];
		if(!empty($wg)) {
			add_image_size( 'rt-size-hard-crop', ''.$wg.'',''.$hg.'', true );
			// $image_size = ('giant-size-hard-crop');
			$image_size = (array(''.$wg.'',''.$hg.''));
		}else{
