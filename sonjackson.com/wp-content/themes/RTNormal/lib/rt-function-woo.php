...
if (  ! function_exists( 'woocommerce_template_loop_product_views' ) ) {
  ...
  function woocommerce_template_loop_product_views() {
    echo "<div class='xem-view'>";
    echo '<a class="views-all" href="' . get_permalink() . '"  title="' . get_the_title() . '">Xem chi tiết </a>';
    echo '<a class="muahang" href="' . get_permalink() . '"  title="' . get_the_title() . '"> Mua hàng </a>';
    echo "</div>";
  }
}
...
if (  ! function_exists( 'woocommerce_template_loop_product_price' ) ) {
  ...
  function woocommerce_template_loop_product_price() {
    ...
    if(!empty($giakm) && !empty($gia) ) {
            echo "<ins> <span> Giá : " . number_format($giakm,0,'','.') . " đ</span></ins>";
    }else{
        echo "<span>";
        if(!empty($gia)) echo "Giá : " .number_format($gia,0,'','.')." đ"; else echo "Giá : Liên Hệ";
        echo "</span>"; 
    }
    echo "</span>";
  }
